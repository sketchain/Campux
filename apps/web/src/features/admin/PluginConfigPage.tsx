...
const PLUGINS: PluginDescriptor[] = [
  ...
  },
  {
    ...
    detailedDescription:
      ...
      "好友申请只放行群成员：收到好友申请后用 get_group_member_info 查询申请人是否在班级群里。在群里按原有随机延迟（30–90 秒）通过；确认不在群里直接拒绝；查询失败（超时或偶发报错）会隔几秒重试一次，仍失败则不自动同意也不拒绝，留给人工在 QQ 里处理，并在审核群提示。以上情况均写入审计日志。\n\n" +
